...
毎日の電気使用量は翌日以降にアカウントのトークン残高から SPX を優先して差し引かれます。ユーザーはこのトークン取引を通じて、自身の使用する電気の種類や価格に関与できます。
毎月 1 日には前月分の精算が行われ、請求額が自動的に計算されます。

### 導入

EDISON の使用にはアカウント作成が必要です。アカウント作成ページは`EDISON 主要画面`を参照してください。
...
当月には前月の余剰トークンは引き継がず、新しく前月の電気使用量と同量の UPX トークンが付与されます。このトークンは新規のアカウント作成同様に 1UPX あたり 27 円で購入したものとして扱われます。

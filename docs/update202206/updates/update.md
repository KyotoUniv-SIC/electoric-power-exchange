...
- 4. 電力価格インフレの改善
  ...
  - 約定価格と約定電気量の日時時系列を表示（27 JPY/kWh を赤の水平線で表示）#262
  - 価格乖離時の市場介入の実装 #260
...
- 5. SPX 購入のインセンティブ付与 未実装
  ...
  - Dashboard への CO2 ランキング通知（本人分のみ）#277
...
<span style="color: red; ">(未実装)</span> Dashboard 内に先月の電気使用量と CO2 排出量の実績値とユーザー全体の平均値を表示する機能の実装を行った。

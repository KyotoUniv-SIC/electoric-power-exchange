...
XRP Ledger バリデーターの詳細
<https://xrpscan.com/validator/nHB2BdLXomCawHvWno5qBtajQvKe1hCpF2e9XGToW92znxaztUn7>
...
## 本システム運営に関する、経理側の仕訳業務

下記の 2 パターンを想定。

① 原則処理

- 月初・月中のトークン販売時に未収入金/雑収入処理。
- 月末に現金を徴収したときは、現金/未収入金処理。

② 簡易処理

- 月末の現金徴収時にまとめて現金/雑収入処理。
...
=>
Message 機能も完成。Admin Dashboard のみ未完成。動作確認も XRP Ledger を含めて、一通りは取れている。
...
- 現状はテストネットで動作するので、アカウント作成時に 1000XRP が付与され、同時に UPX、SPX のトラストラインも作成される。（試験的に 50UPX, 50SPX を付与）

- メインネットでは XRP Ledger アカウントを作成、学生に作成したアドレス宛に XRP を入金して貰う必要がある。
...
コールドウォレットの秘密鍵は追加でトークンを発行する際に必要になるので、CauchyE 担当者でのオフライン管理とする予定。XRP Ledger アカウントの秘密鍵は Google Firebase に暗号化して置く予定。

...
import { IRenewableAskSettingInfrastructureService } from './renewable-ask-setting.service';
...
export class RenewableAskSettingInfrastructureService
  implements IRenewableAskSettingInfrastructureService {

  constructor(private readonly firestore: Firestore) {}

  collection(...queryConstraints: QueryConstraint[]) {
    const ref = collection(this.firestore, RenewableAskSettingFirestore.collectionPath());

    return (queryConstraints.length > 0
      ? query(ref, ...queryConstraints) : ref).withConverter(RenewableAskSettingFirestore.converter);
  }

  collectionGroup(...queryConstraints: QueryConstraint[]) {
    const ref = collectionGroup(this.firestore, RenewableAskSettingFirestore.collectionID);
  ...
  document(id?: string) {
    ...
    return (id ? doc(this.firestore, ref.path, id) : doc(this.firestore, ref.path, autoID())).withConverter(RenewableAskSettingFirestore.converter);
  }

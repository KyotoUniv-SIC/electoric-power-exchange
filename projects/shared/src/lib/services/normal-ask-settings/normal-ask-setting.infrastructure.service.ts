...
import { INormalAskSettingInfrastructureService } from './normal-ask-setting.service';
...
export class NormalAskSettingInfrastructureService
  implements INormalAskSettingInfrastructureService {

  constructor(private readonly firestore: Firestore) {}

  collection(...queryConstraints: QueryConstraint[]) {
    const ref = collection(this.firestore, NormalAskSettingFirestore.collectionPath());

    return (queryConstraints.length > 0
      ? query(ref, ...queryConstraints) : ref).withConverter(NormalAskSettingFirestore.converter);
  }

  collectionGroup(...queryConstraints: QueryConstraint[]) {
    const ref = collectionGroup(this.firestore, NormalAskSettingFirestore.collectionID);
  ...
  document(id?: string) {
    ...
    return (id ? doc(this.firestore, ref.path, id) : doc(this.firestore, ref.path, autoID())).withConverter(NormalAskSettingFirestore.converter);
  }

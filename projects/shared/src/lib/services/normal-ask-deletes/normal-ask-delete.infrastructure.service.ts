...
export class NormalAskDeleteInfrastructureService
  implements INormalAskDeleteInfrastructureService {
  ...
  collection(...queryConstraints: QueryConstraint[]) {
    ...
    return (queryConstraints.length > 0
      ? query(ref, ...queryConstraints) : ref).withConverter(NormalAskDeleteFirestore.converter);
  }
  ...
  document(id?: string) {
    ...
    return (id ? doc(this.firestore, ref.path, id) : doc(this.firestore, ref.path, autoID())).withConverter(NormalAskDeleteFirestore.converter);
  }

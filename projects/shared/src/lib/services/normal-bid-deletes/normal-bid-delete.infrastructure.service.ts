...
export class NormalBidDeleteInfrastructureService implements INormalBidDeleteInfrastructureService {
  ...
  collection(...queryConstraints: QueryConstraint[]) {
    ...
    return (queryConstraints.length > 0 ? query(ref, ...queryConstraints) : ref).withConverter(NormalBidDeleteFirestore.converter);
  }
  ...
  document(id?: string) {
    ...
    return (id ? doc(this.firestore, ref.path, id) : doc(this.firestore, ref.path, autoID())).withConverter(
      NormalBidDeleteFirestore.converter,
    );
  }

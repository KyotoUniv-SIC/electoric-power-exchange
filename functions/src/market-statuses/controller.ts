...
import { MarketStatusFirestore } from '@local/common';
...
module.exports.onCreate = functions.firestore.document(MarketStatusFirestore.virtualPath).onCreate(async (snapshot, context) => {
...
module.exports.onUpdate = functions.firestore.document(MarketStatusFirestore.virtualPath).onUpdate(async (snapshot, context) => {
  if (await isTriggeredOnce(context.eventId)) {
    return;
  }
...
module.exports.onDelete = functions.firestore.document(MarketStatusFirestore.virtualPath).onDelete(async (snapshot, context) => {
  if (await isTriggeredOnce(context.eventId)) {
    return;
  }

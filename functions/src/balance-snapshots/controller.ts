...
import { BalanceSnapshotFirestore } from '@local/common';
...
module.exports.onCreate = functions.firestore.document(BalanceSnapshotFirestore.virtualPath).onCreate(async (snapshot, context) => {
...
module.exports.onUpdate = functions.firestore.document(BalanceSnapshotFirestore.virtualPath).onUpdate(async (snapshot, context) => {
  if (await isTriggeredOnce(context.eventId)) {
    return;
  }
...
module.exports.onDelete = functions.firestore.document(BalanceSnapshotFirestore.virtualPath).onDelete(async (snapshot, context) => {
  if (await isTriggeredOnce(context.eventId)) {
    return;
  }

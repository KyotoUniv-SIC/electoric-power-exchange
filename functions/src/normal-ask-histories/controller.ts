...
module.exports.onCreate = functions.firestore.document(NormalAskHistoryFirestore.virtualPath).onCreate(async (snapshot, context) => {
...
module.exports.onUpdate = functions.firestore.document(NormalAskHistoryFirestore.virtualPath).onUpdate(async (snapshot, context) => {
  if (await isTriggeredOnce(context.eventId)) {
    return;
  }
...
module.exports.onDelete = functions.firestore.document(NormalAskHistoryFirestore.virtualPath).onDelete(async (snapshot, context) => {
  if (await isTriggeredOnce(context.eventId)) {
    return;
  }

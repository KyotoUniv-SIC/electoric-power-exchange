...
module.exports.onCreate = functions.firestore.document(PrimaryAskFirestore.virtualPath).onCreate(async (snapshot, context) => {
...
module.exports.onUpdate = functions.firestore.document(PrimaryAskFirestore.virtualPath).onUpdate(async (snapshot, context) => {
  if (await isTriggeredOnce(context.eventId)) {
    return;
  }
...
module.exports.onDelete = functions.firestore.document(PrimaryAskFirestore.virtualPath).onDelete(async (snapshot, context) => {
  if (await isTriggeredOnce(context.eventId)) {
    return;
  }

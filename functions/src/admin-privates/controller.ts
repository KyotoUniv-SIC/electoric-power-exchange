...
export const onCreate = functions.firestore.document(AdminPrivateFirestore.virtualPath).onCreate(async (snapshot, context) => {
...
export const onUpdate = functions.firestore.document(AdminPrivateFirestore.virtualPath).onUpdate(async (snapshot, context) => {
  if (await isTriggeredOnce(context.eventId)) {
    return;
  }
...
export const onDelete = functions.firestore.document(AdminPrivateFirestore.virtualPath).onDelete(async (snapshot, context) => {
  if (await isTriggeredOnce(context.eventId)) {
    return;
  }

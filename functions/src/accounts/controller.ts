...
module.exports.onCreate = f.firestore.document(AccountFirestore.virtualPath).onCreate(async (snapshot, context) => {
...
module.exports.onUpdate = f.firestore.document(AccountFirestore.virtualPath).onUpdate(async (snapshot, context) => {
  if (await isTriggeredOnce(context.eventId)) {
    return;
  }
...
module.exports.onDelete = f.firestore.document(AccountFirestore.virtualPath).onDelete(async (snapshot, context) => {
  if (await isTriggeredOnce(context.eventId)) {
    return;
  }
